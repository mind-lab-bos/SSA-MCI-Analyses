...
for (i in 1:nrow(post_df)) {
    if(post_df$SUBID[i]=="190904GHER") {
    post_df$SUBID[i] = "190904GHER"
  } else {
    if(post_df$SUBID[i]=="220914GHER") {
    post_df$SUBID[i] = "220914GHER"
    } else {
    post_df$SUBID[i]<-gsub('[[:digit:]]+', '',post_df$SUBID[i])
  }
  }
}
...
TRs_SSA<-merge(TRs, SSAdata, by.x="order",by.y="stim_order")
```
...
TRs<- (seq(1,1440,1))
TRs_roi<- as.data.frame(rep(TRs, 35))
names(TRs_roi) <- "TR"
...
attempt <- merge(TRs_SSA, roi_dataID, by.x=c("TR", "SUBID"), by.y=c("TR","SUBID"))
...
roi_timeseries <- write.csv(attempt, file="roidata2.csv")
```
