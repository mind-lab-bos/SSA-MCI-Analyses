...
test <- lmer(V1 ~ time*cognitive_health + (1|SUBID), data=mpfc_reward)
anova(test)
...
reward_justMCI <- subset(reward, cognitive_health=="MCI/SCD")
test <- lmer(V1 ~ time + (1|SUBID), data=reward_justMCI)

emm_s.t <- emmeans(test, pairwise ~ time | cognitive_health,lmer.df = "satterthwaite",lmerTest.limit = 4320)
contrast(emm_s.t[[1]],interaction = c("poly")) #this gives us simple slopes
contrast(emm_s.t[[1]],interaction = c("poly", "consec"), by = NULL) #this gives us the interaction contrast
...
test <- lmer(V1 ~ time*cognitive_health + (1|SUBID), data=reward_auditory)
anova(test)
