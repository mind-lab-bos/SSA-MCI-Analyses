...
childhood <- as.data.frame(childhood)
childhood[childhood == 1] <- NA
...
childhood_auditory <- data.frame(childhood_auditory)
...
childhood_reward <- data.frame(childhood_reward)
...
mean(childhood_auditory_reward, na.rm=TRUE)
childhood_auditory_reward <- data.frame((childhood_auditory_reward))
...
adolescence <- as.data.frame(adolescence)
adolescence[adolescence == 1] <- NA
...
adolescence_auditory <- as.data.frame(adolescence_auditory)
...
adolescence_reward <- as.data.frame(adolescence_reward)
...
mean(adolescence_auditory_reward, na.rm=TRUE)
adolescence_auditory_reward <- data.frame(adolescence_auditory_reward)
...
corrplot(youngadulthood, tl.pos='n', col.lim = c(-0.5,1), col=colorRampPalette(c("blue","white","red"))(200), type="upper", cl.cex = 3.5,method="color")
dev.off()

youngadulthood <- as.data.frame(youngadulthood)
youngadulthood[youngadulthood == 1] <- NA
...
youngadult_auditory <- as.data.frame(youngadult_auditory)
...
youngadult_reward <- as.data.frame(youngadult_reward)
...
mean(youngadult_auditory_reward, na.rm=TRUE)
youngadult_auditory_reward <- data.frame(youngadult_auditory_reward)
...
adult_reward <- as.data.frame(adult_reward)
...
mean(adult_auditory_reward, na.rm=TRUE)
adult_auditory_reward <- data.frame(adult_auditory_reward)
...
#mpfc_auditory_mpfc <-
  ...
  scale_x_discrete(labels=c("Childhood (0-11)", "Adolescence (12-18)", "Young\nAdulthood (19-25)",
                            "Adulthood (26-45)")) +
     geom_signif(
    comparisons = list(c("childhood_mpfc", "adulthood_mpfc")),
...
auditory <-
  ...
  scale_x_discrete(labels=c("Childhood (0-11)", "Adolescence (12-18)", "Young\nAdulthood (19-25)",
                            "Adulthood (26-45)")) +
  ggtitle("Average Within-Auditory Network Connectivity")

ggsave('../results/auditory.png', auditory, width=10)
...
#reward <-
  ggplot(reward, aes(x=time, y=correlation)) +
  stat_summary(fun = 'mean',geom="line", group=1) +
  stat_summary(fun.data = 'mean_se', geom = 'errorbar', width = 0.2) +
  stat_summary(fun.data = 'mean_se', geom = 'pointrange') +
  ylab("Average Correlation Coefficient") +
  ...
  scale_x_discrete(labels=c("Childhood (0-11)", "Adolescence (12-18)", "Young\nAdulthood (19-25)",
                            "Adulthood (26-45)")) +
  ggtitle("Average Within-Reward Region Connectivity") +
  geom_signif(
    comparisons = list(c("childhood", "adulthood")),
    map_signif_level = TRUE,
    y_position=0.325
  ) 
  
ggsave('../results/reward.png', reward, width=10)
...
reward_auditory <-
  ...
  scale_x_discrete(labels=c("Childhood (0-11)", "Adolescence (12-18)", "Young\nAdulthood (19-25)",
                            "Adulthood (26-45)")) +
  ggtitle("Average Connectivity Between Auditory & Reward Regions") +
  geom_signif(
  ...
  geom_signif(
    comparisons = list(c("childhood", "young_adult")),
    map_signif_level = TRUE,
    y_position=0.25
  ) +
  geom_signif(
    comparisons = list(c("childhood", "adult")),
    map_signif_level = TRUE,
    y_position=0.25
  ) 
  
ggsave('../results/reward_auditory.png', reward_auditory, width=10)
